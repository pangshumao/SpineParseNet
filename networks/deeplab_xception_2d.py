...
class SeparableConv2d_same(nn.Module):
    def __init__(self, inplanes, planes, kernel_size=3, stride=1, dilation=1, bias=False, padding=0, normalization='bn',
                 num_groups=8):
...
#             rep.append(nn.BatchNorm2d(planes))
        ...
        if stride != 1:
            self.block2_lastconv = nn.Sequential(*[self.relu,SeparableConv2d_same(planes, planes, 3, stride=stride,
                                                dilation=dilation, normalization=normalization, num_groups=num_groups)])
...
class DeepLabv3_plus_2d(nn.Module):
    def __init__(self, nInputChannels=3, n_classes=21, os=16, pretrained=False, _print=True, final_sigmoid=False,
        ...
        super(DeepLabv3_plus_2d, self).__init__()
        ...
        #                              )
        # self.semantic = nn.Conv2d(128, n_classes, kernel_size=1, stride=1)

        self.decoder = nn.Sequential(Decoder_module(152, 256, normalization=normalization, num_groups=num_groups),
    ...
    def train_fixbn(self, mode=True, freeze_bn=True, freeze_bn_affine=False):
        ...
        Returns:
            Module: self
        """
        super(DeepLabv3_plus_2d, self).train(mode)
        if freeze_bn:
...
if __name__ == "__main__":
    ...
    model = DeepLabv3_plus_2d(nInputChannels=3, n_classes=21, os=16, pretrained=False, _print=True)
