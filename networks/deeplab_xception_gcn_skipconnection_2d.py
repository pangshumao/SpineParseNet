...
from networks.global_reasoning_unit import GloRe_Unit_2D
...
#             rep.append(nn.BatchNorm2d(planes))
        ...
        if stride != 1:
            self.block2_lastconv = nn.Sequential(*[self.relu,SeparableConv2d_same(planes, planes, 3, stride=stride,dilation=dilation)])
...
class DeepLabv3_plus_gcn_skipconnection_2d(nn.Module):
    def __init__(self, nInputChannels=3, n_classes=21, os=16, pretrained=False, _print=True, final_sigmoid=False,
        ...
        if self.gcn_mode == 0:
            ...
            self.global_reasoning_unit = GloRe_Unit_2D(num_in=128, num_s=128, num_n=64)
            ...
            self.graph_2_fea = gcn.My_Graph_to_Featuremaps(hidden_layers=hidden_layers, output_channels=128, dimension=2)

        self.skip_conv = nn.Sequential(*[nn.Conv2d(128, 128, kernel_size=1),
                                         nn.ReLU(True)])
    ...
    def forward(self, img, unary=None, return_feat=False, **kwargs):
        input = img
        ...
        if return_feat:
            # feature = F.interpolate(low_level_features_2, size=input.size()[2:], mode='bilinear', align_corners=True)
            if not self.training:
                x = self.final_activation(final_conv)
    ...
    def train_fixbn(self, mode=True, freeze_bn=True, freeze_bn_affine=False):
        ...
        Returns:
            Module: self
        """
        super(DeepLabv3_plus_gcn_skipconnection_2d, self).train(mode)
        if freeze_bn:
...
if __name__ == "__main__":
    ...
    image = torch.randn(2, 3, 256, 512) * 255
    image = image.to(device)
    model = DeepLabv3_plus_gcn_skipconnection_2d(nInputChannels=3, n_classes=21, os=16, pretrained=False, _print=True)
