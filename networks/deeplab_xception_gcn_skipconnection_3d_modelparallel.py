...
class DeepLabv3_plus_gcn_skipconnection_3d_subnet1(nn.Module):
    def __init__(self, os=16,):
        super(DeepLabv3_plus_gcn_skipconnection_3d_subnet1, self).__init__()
    ...
    def forward(self, x, low_level_features_4):
        '''
...
class DeepLabv3_plus_gcn_skipconnection_3d_subnet2(nn.Module):
...
class DeepLabv3_plus_gcn_skipconnection_3d_subnet3(nn.Module):
    def __init__(self, n_classes, final_sigmoid):
        super(DeepLabv3_plus_gcn_skipconnection_3d_subnet3, self).__init__()

        self.decoder2 = nn.Sequential(Decoder_module(192, 128),
                                      Decoder_module(128, 128))
    ...
    def forward(self, x, input, fea_logit, is_training):
        '''
        ...
        if not is_training:  # test
            activation = self.final_activation(final_conv)
            return activation, final_conv, fea_logit
        else:  # training
...
class DeepLabv3_plus_gcn_skipconnection_3d(nn.Module):
    def __init__(self, nInputChannels=3, n_classes=20, os=16, pretrained=False, _print=True, final_sigmoid=False,
                 hidden_layers=128, gcn_mode=0, devices=None):
        if _print:
        ...
        if len(self.devices) > 1:
            self.xception_features = Xception(nInputChannels, os, pretrained).to(self.devices[1])
        else:
            self.xception_features = Xception(nInputChannels, os, pretrained).to(self.devices[0])

        if len(self.devices) == 1:
            self.sub_net1 = DeepLabv3_plus_gcn_skipconnection_3d_subnet1(os).to(self.devices[0])

            self.sub_net2 = DeepLabv3_plus_gcn_skipconnection_3d_subnet2(n_classes, hidden_layers, gcn_mode,
                                                                         self.devices[0]).to(self.devices[0])
            self.sub_net3 = DeepLabv3_plus_gcn_skipconnection_3d_subnet3(n_classes, final_sigmoid).to(self.devices[0])
        else:
            self.sub_net1 = DeepLabv3_plus_gcn_skipconnection_3d_subnet1(os).to(self.devices[0])

            self.sub_net2 = DeepLabv3_plus_gcn_skipconnection_3d_subnet2(n_classes, hidden_layers, gcn_mode,
                                                                         self.devices[2]).to(self.devices[2])
            self.sub_net3 = DeepLabv3_plus_gcn_skipconnection_3d_subnet3(n_classes, final_sigmoid).to(self.devices[3])


    def forward(self, input):
        if len(self.devices) > 1:
            input = input.to(self.devices[1])
        x, low_level_features_2, low_level_features_4 = self.xception_features(input)

        if len(self.devices) > 1:
            x = x.to(self.devices[0])
            low_level_features_4 = low_level_features_4.to(self.devices[0])
        x = self.sub_net1(x, low_level_features_4)

        if len(self.devices) > 1:
            x = x.to(self.devices[2])
            low_level_features_2 = low_level_features_2.to(self.devices[2])
        x, fea_logit = self.sub_net2(x, low_level_features_2)

        if len(self.devices) > 1:
            x = x.to(self.devices[3])
            fea_logit = fea_logit.to(self.devices[3])
        results = self.sub_net3(x, input, fea_logit, self.training)

        if len(self.devices) > 1:
            results = [result.to(self.devices[0]) for result in results]

        if len(results) == 2:
            # training
            final_conv, fea_logit = results
            return final_conv, fea_logit
        else:
            # test
            activation, final_conv, fea_logit = results
            return activation, final_conv, fea_logit
